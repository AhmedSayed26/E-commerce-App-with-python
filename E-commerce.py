...
class Category:
    ...
    def add_category(self, category_name, user_email):
        ...
        messagebox.showinfo("Successfully", f"Category {category_name} added successfully")
    ...
    def edit_category(self, old_category_name, new_category_name, admin_email):
        ...
        if new_category_name in self.categories:
            messagebox.showerror("Error", f"Category '{new_category_name}' already exist")
            return

        self.categories[old_category_name] = self.categories[new_category_name]
        self.save_categories()
        messagebox.showinfo("Successfully", f"Category '{old_category_name}' renamed to '{new_category_name}' successfully")
    ...
    def delete_category(self, category_name, admin_email):
        ...
        if category_name not in self.categories:
            messagebox.showerror("Error", f"Category '{category_name}' doesn't exist")
            return

        del self.categories[category_name]
        self.save_categories()
        messagebox.showinfo("Successfully", f"Category '{category_name}' deleted successfully")
    ...
    def show_categories(self):
        ...
        if not self.categories:
            ...
            tk.Label(categories_window, text="Categories",font="Bold").pack(bady=5)
            for category_name in self.categories:
                tk.Label(categories_window, text=f"- {category_name}",font="Bold").pack(bady=5)
...
class Item:
    ...
    def add_item_to_category(self, category_name, admin_email):
        ...
        messagebox.showinfo("Successfully", f"Item {self.name} added to {category_name}.")
    ...
    def delete_item_from_category(self, category_name, item_name, admin_email):
        ...
        for item in category_items:
            if item['name'] == item_name:
                ...
                category.save_categories()
                messagebox.showinfo("Successfully", f"item {item_name} deleted from {category_name}.")
                return
        messagebox.showerror("Error", f"item {item_name} not found in {category_name}")
    ...
    def edit_item(self, category_name, old_item_name, new_name=None, new_price=None, new_brand=None, new_year=None, new_discount = None,
        ...
        for item in category_items:
            if item['name'] == old_item_name:
                ...
                self.save_categories()
                messagebox.showinfo("Successfully", f"item '{old_item_name}' updated successfully")
                return

        if not item_found:
            messagebox.showerror("Error", f"item '{old_item_name}' not found in '{category_name}'.")
    ...
    def search(self, category_name, item_name):
        ...
        if search_item:
            for item in search_item:
                messagebox.showinfo("Search Results", f"{item['name']} \n {item['price']} \n {item['brand']} \n {item['model_year']} \n {item['discount']}")
        else:
...
class Cart:
    ...
    def calculate_total(self, governorate):
        ...
        messagebox.showinfo("Total Cost",
                            f"Total: {total}\ndelivery Fee: {delivery_fee},\nTotal with delivery: {total_with_delivery}")
    ...
    def search(self, item_name):
        ...
        if search_item:
            for item in search_item:
                messagebox.showinfo("search result", f"{item['name']} \n{item['price']} \n{item['brand']} \n{item['model_year']}\n{item['discount']}")
        else:
...
def register():
    ...
    def save_registration():
        ...
        if email in users:
            error_label.config(text="email already exist")
        else:
            users[email] = {
                "name": name,
                "id": user_id,
            ...
            save_users(users)
            messagebox.showinfo("Successfully", "Registration successfully")
...
def admin_panel():
    ...
    def add_category():
        ...
        tk.Label(category_window, text="Category_Name", font="Bold",bg="lightblue").pack()
    ...
    def delete_item(category_name, items_window):
        def submit_delete_item():
            ...
            messagebox.showinfo("Successfully", f"'{item_name}' deleted from '{category_name}'")
            delete_item_window.destroy()
            refresh_items_window(category_name, items_window)

        delete_item_window = tk.Toplevel()
        delete_item_window.title("Delete Item")
        tk.Label(delete_item_window, text="Item_Name", font="Bold",bg="lightblue").pack()
    ...
    def edit_item(category_name,items_window):
        ...
        tk.Label(edit_item_window, text="new name (leave empty if you want)", font="Bold",bg="lightblue").pack()
        new_name_entry = tk.Entry(edit_item_window)
        new_name_entry.pack()
        tk.Label(edit_item_window, text="new price (leave empty if you want)", font="Bold",bg="lightblue").pack()
        new_price_entry = tk.Entry(edit_item_window)
        new_price_entry.pack()
        tk.Label(edit_item_window, text="new brand (leave empty if you want)", font="Bold",bg="lightblue").pack()
        new_brand_entry = tk.Entry(edit_item_window)
        new_brand_entry.pack()
        tk.Label(edit_item_window, text="new model Year (leave empty if you want)", font="Bold",bg="lightblue").pack()
        new_year_entry = tk.Entry(edit_item_window)
        new_year_entry.pack()
        tk.Label(edit_item_window, text="new discount (leave empty if you want)", font="Bold",bg="lightblue").pack()
    ...
    def delete_category():
        ...
        tk.Label(delete_category_window, text="Category_Name", font="Bold",bg="lightblue").pack()
    ...
    def edit_category():
        def submit_edit_category():
            ...
            messagebox.showinfo("Successfully", f"Category name '{old_category_name}' updated to '{new_category_name}'")
        ...
        tk.Button(edit_category_window, text="Edit", fg="red",font="Bold", command=submit_edit_category).pack(pady=5)


    def refresh_admin_panel():
        for widget in admin_window.winfo_children():
            widget.destroy()
        tk.Label(admin_window, text="Welcome to Admin Panel :)",font="Bold",fg="blue",pady=10).pack(pady=7)
...
def show_category_items(category_name, user_email):
    ...
    search_sort_frame = tk.Frame(items_window, bg="lightblue")
    search_sort_frame.pack(pady=5)

    tk.Label(search_sort_frame, text="Search Item:", bg="lightblue", font="Bold").pack(side=tk.LEFT, padx=5)
    ...
    tk.Button(search_sort_frame, text="Search", fg="white", width=10, font="Bold", bg="gray", command=search).pack(side=tk.LEFT, padx=5)

    tk.Button(search_sort_frame, text="Sort by Price", fg="white", width=15, font="Bold", bg="gray", command=lambda: sort_items("price")).pack(side=tk.LEFT, padx=5)
    tk.Button(search_sort_frame, text="Sort by Model Year", fg="white", width=15, font="Bold", bg="gray", command=lambda: sort_items("model_year")).pack(side=tk.LEFT, padx=5)
    tk.Button(search_sort_frame, text="Sort by Name (Z-A)", fg="white", width=15, font="Bold", bg="gray", command=lambda: sort_items("name")).pack(side=tk.LEFT, padx=5)
